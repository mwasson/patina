...
impl Tile {
    ...
    #[allow(dead_code)]
    pub fn render(&mut self, mapper: &dyn Mapper) -> [u8; 4 * 8 * 8] {
        let mut out = [0; 4 * 8 * 8];
    ...
    #[allow(dead_code)]
    pub fn stamp(&mut self, mapper: &dyn Mapper, write_buffer: &mut [u8], width: usize, x: usize, y: usize) {
    ...
    pub fn pixel_intensity(&mut self, mapper: &dyn Mapper, x: u8, y: u8) -> u8 {
    ...
    fn populate_cache(&mut self, mapper: &dyn Mapper, y: u8) {
        if self.cached_y != y {
            self.cached_y = y;
